...
from pyramid.neutral_zone.readers.open_ephys_session import OpenEphysSessionSignalReader
...
expected_sample_frequency = 40000.0
    ...
    with OpenEphysSessionSignalReader(binary_session_path) as reader:
        ...
        assert reader.get_initial() == {
            reader.result_name: SignalChunk.empty(
                sample_frequency=expected_sample_frequency,
...
def test_signal_default_read_binary_format(binary_session_path):
    with OpenEphysSessionSignalReader(binary_session_path) as reader:
        ...
        assert np.array_equal(first_sample, expected_first_sample)
        assert first_chunk.sample_frequency == expected_sample_frequency
        assert first_chunk.first_sample_time == 0.0
        assert first_chunk.channel_ids == expected_channel_ids
        ...
        for read_index in range(28):
            ...
            assert next_chunk.sample_frequency == expected_sample_frequency
            assert next_chunk.channel_ids == expected_channel_ids
        ...
        assert last_chunk.sample_frequency == expected_sample_frequency
        assert last_chunk.channel_ids == expected_channel_ids
